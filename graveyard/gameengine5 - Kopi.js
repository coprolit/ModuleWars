                ...
                //window.requestAnimationFrame(function(){
                    ...
                    for(var i = 0; i < items.length; i++){
                        ...
                        if(gameObjectsModel[username]){
                            ...
                            gameObjectsModel[username].GetUserData().lastServerState.x = posX;
                            gameObjectsModel[username].GetUserData().lastServerState.y = posY;
                            gameObjectsModel[username].GetUserData().lastServerState.angle = angle;
                            gameObjectsModel[username].GetUserData().lastServerState.vx = velX;
                            gameObjectsModel[username].GetUserData().lastServerState.vy = velY;
                            /*
        ...
        if(b2world.GetBodyCount() > 1){
            for(var body = b2world.GetBodyList(); body; body = body.GetNext()){
                if (body.IsActive() && typeof body.GetUserData() !== 'undefined' && body.GetUserData() !== null) {
                    // smooth differences in client and last saved server state:
                    var lastServerState = body.GetUserData().lastServerState;

                    var xDiff = lastServerState.x - body.GetPosition().x;
                    var xSmoothed = body.GetPosition().x + xDiff / 2;
                    var yDiff = lastServerState.y - body.GetPosition().y;
                    var ySmoothed = body.GetPosition().y + yDiff / 2;
                    body.SetPosition(new b2Vec2(xSmoothed, ySmoothed));

                    var angleDiff = lastServerState.angle - body.GetAngle();
                    var angleSmoothed = body.GetAngle() + angleDiff / 2;
                    body.SetAngle(angleSmoothed);

                    var vxDiff = lastServerState.vx - body.GetLinearVelocity().x;
                    var vxSmoothed = body.GetLinearVelocity().x + vxDiff / 2;
                    var vyDiff = lastServerState.vy - body.GetLinearVelocity().y;
                    var vySmoothed = body.GetLinearVelocity().y + vyDiff / 2;
                    body.SetLinearVelocity(new b2Vec2(vxSmoothed, vySmoothed));
                }
            }
        }
